...
<%! 
	...
	private void registarConexaoSucesso(HttpSession session, String idSeparador, Socket socket, client.Stub stub) {
...
<%
	...
	// 🚿 "Login" NÃO pode apagar a sessão de rede! Apenas "Sair" limpa tudo.
	if ("Sair".equals(acao)) {
    ...
    if (fase.equals("CONECTAR")) {
        ...
        try {
            // 1. Tentativa padrão de conexão
            socket = new Socket(hostSessao, portoFinal);
            stub = new Stub(socket);
            
            registarConexaoSucesso(session, idSeparador, socket, stub);
            fase = "LOGIN"; // Atualiza a variável local para o HTML saber o que desenhar
        } catch (Exception e) {
            // 2. Se falhar e for máquina local, lança a Thread do Servidor
            if ("localhost".equals(hostSessao) || "127.0.0.1".equals(hostSessao)) {
                try {
                    Thread t = new Thread(() -> {
                        try {
                    ...
                    // 🔄 Ciclo de tentativas de reconexão dinâmicas (Máximo 4 vezes)
                    int tentativas = 0;
                    boolean conectadoComSucesso = false;
                    
                    while (tentativas < 4 && !conectadoComSucesso) {
                        try {
                            Thread.sleep(500); // Espera que o porto abra
                            socket = new Socket(hostSessao, portoFinal);
                            stub = new Stub(socket);
                            
                            registarConexaoSucesso(session, idSeparador, socket, stub);
                            fase = "LOGIN"; // ✔️ Define a fase para o HTML renderizar
                            conectadoComSucesso = true;
                        } catch (Exception erroSocket) {
                            tentativas++;
                        }
                    }
                    
                } catch (Exception erroAoLancar) {
                    erroGrave = "Falha ao acordar o servidor local: " + erroAoLancar.getMessage();
                }
            }
            
            // 3. Se esgotou as tentativas e NÃO conseguiu ligar, volta para a Configuração de IP
            if (!fase.equals("LOGIN")) {
                if (erroGrave == null) {
    ...
    <% } else if (fase.equals("LOGIN")) { %>
			...
			<div class="lista-utilizadores">
			    ...
			    <ul class="lista-limpa">
			        ...
			        <li><a title="Acede aos utilizadores registados" href="users.jsp">Registo de Utilizadores</a></li>
			    </ul> 
			</div><br>
